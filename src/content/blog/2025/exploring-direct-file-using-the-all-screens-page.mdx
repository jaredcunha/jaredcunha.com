...
Once that is up and running, visit `http://localhost:3000/df/file/all-screens/index.html`. _I will note_ that it could use performance improvements, so as you open and close the menu or toggle display modes, there may be a slight lag. Also for me, when using Chrome, I would have to open this in incognito mode due to a redirect (index.html is required in the URL). Other browsers seem to work fine.
...
<Image
  src="/images/built-a-good-thing.jpg"
  alt="Graphic with pink flamingos at the top and black leaf branches forming a wreath border around text that reads 'WE BUILT A REALLY GOOD THING, FRIENDS' in bold black letters, with 'DF4LIFE' in pink text at the bottom."
  className="narrow"
/>
